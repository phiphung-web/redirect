...
new_block = """app.get(\"/campaigns/:id/report/v2\", checkAuth, async (req, res) => {
  ...
  const rCamp = await db.query(SELECT * FROM campaigns WHERE id=, [
    campId,
  ]);
  ...
  if (preset === \"this_week\") {
    ...
    const earliest = await db.query(
      SELECT MIN(created_at) AS min_date FROM traffic_logs WHERE campaign_id=,
      [campId]
    );
    const minRaw =
  ...
  const stats = await db.query(
        ...
        SELECT date_trunc(::text, created_at) as bucket, 
  ...
  const logs = await db.query(
    SELECT * FROM traffic_logs WHERE campaign_id= AND created_at >=  AND created_at <  ORDER BY id DESC LIMIT 50,
    [campId, start, end]
  );
